...
import useActivityCreate from '@/composables/useActivityCreate'
...
useUnsavedLeaveGuard(isDirty, () =>
  updateActivityData({ id: routeId.value, payload: formModel.value }),
)
...
const fetchActivityById = async (id: string) => {
  try {
    const res = await activityEdit.fetchActivityById(id)
    if (res?.status === 'success') {
...
<template>
  <div
    class="min-h-screen bg-[var(--color-bg-page)] py-24px min-w-768px"
    v-loading="isFetching === true || isUpdating === true"
