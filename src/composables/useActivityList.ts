...
const useActivityList = ()=>{
  ...
  const changeFilter = async(status: FilterStatus)=>{
    ...
    const params = {filterStatus:activitiesData.value.filters, currentPage:activitiesData.value.pageInfo.currentPage}
  ...
  const setOfflineManualConfirm = async({ id, status }: { id: string, status: Status }):Promise<ActivityResponse | undefined> => {
